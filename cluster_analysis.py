...
cluster_stats_df.to_csv('cluster_target_amount_stats.csv', encoding='utf-8-sig')
print(f"\nクラスタ統計情報を 'cluster_target_amount_stats.csv' に保存しました。")

# 範囲別統計
range_stats_df = pd.DataFrame(overall_range_stats).T
range_stats_df.to_csv('range_target_amount_stats.csv', encoding='utf-8-sig')
print(f"範囲別統計情報を 'range_target_amount_stats.csv' に保存しました。")
...
for cluster_id in sorted(df_analysis['cluster'].unique()):
    ...
    axes[1,0].scatter(cluster_data['AVG_MONTHLY_POPULATION'], 
                     ...
                     alpha=0.6, label=f'クラスタ {cluster_id}')
...
plt.tight_layout()
plt.savefig('cluster_target_amount_analysis.png', dpi=300, bbox_inches='tight')
plt.show()
...
plt.tight_layout()
plt.savefig('cluster_distribution_by_range.png', dpi=300, bbox_inches='tight')
plt.show()
...
print(f"- cluster_target_amount_stats.csv: 各クラスタの統計情報")
print(f"- range_target_amount_stats.csv: 範囲別統計情報")
print(f"- correlation_analysis.png: 相関分析の可視化")
print(f"- clustering_analysis.png: クラスタ数決定の分析")
print(f"- clustering_result.png: クラスタリング結果")
print(f"- cluster_target_amount_analysis.png: クラスタ別の詳細分析")
print(f"- cluster_distribution_by_range.png: 範囲別クラスタ分布")
